...
st.title("📋 调单管理系统")
st.warning("✅ 测试版本 v1.0.6 - 2026-07-23（对客RFI发信）")
...
# ============================================================
    ...
    📌 使用说明：
    1. 选择调单场景 → 自动加载对应的模板
    2. 填写收件人、抄送、主题，编辑草稿内容
    3. 点击「打开邮件客户端撰写」→ 调起网易邮箱等本地客户端并自动填入
    """)
    ...
    with col2:
        ...
        edited_content = st.text_area("草稿内容（可直接编辑修改）", value=default_content, height=320, key="rfi_draft")
        ...
        with mail_col1:
            mail_to = st.text_input(
                "收件人 *",
                placeholder="sales@example.com，多个用逗号分隔",
                key="rfi_mail_to",
            )
        with mail_col2:
        ...
        with btn_col1:
            st.link_button(
                ...
                mailto_url,
                type="primary",
                help="调起系统默认邮件客户端（网易邮箱客户端需设为默认）",
            )
        with btn_col2:
            st.link_button(
                "🌐 打开网易网页邮箱",
                email_defaults["webmail_url"],
                help="网页版无法自动带入正文，需手动粘贴",
            )
        with btn_col3:
            if st.button("📋 复制正文"):
                st.session_state["rfi_copy_hint"] = edited_content
                st.success("✅ 正文已准备好，请按 Ctrl+C 复制下方文本框内容")
        with btn_col4:
            if st.button("🔄 重置"):
                st.rerun()
        if body_truncated:
            st.warning("草稿正文较长，已通过 mailto 填入收件人/抄送/主题。正文请点「复制正文」后粘贴到邮件中。")
        if not normalize_email_list(mail_to):
            st.info("💡 请先填写收件人，再点击「打开邮件客户端撰写」。")
        st.caption("说明：Windows 上请将 **网易邮箱大师/网易企业邮箱客户端** 设为默认邮件程序，mailto 链接才会自动打开并填入字段。")
