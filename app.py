...
st.title("📋 调单管理系统")
st.warning("✅ 测试版本 v1.0.15 - 2026-07-24（内置通讯录兜底）")
...
try:
    from rfi_contacts_embed import get_embedded_contacts
except ImportError:
    get_embedded_contacts = None
...
def load_contacts_from_csv():
    ...
    if get_embedded_contacts:
        contacts = get_embedded_contacts()
        if contacts:
            return contacts, "内置通讯录", None

    return [], None, "未找到通讯录文件，请提交 rfi_contacts.json / rfi_contacts.csv / rfi_contacts_embed.py 到 Git"
...
def get_rfi_contacts_meta():
    ...
    json_path = resolve_contacts_json_path()
    csv_path = resolve_contacts_csv_path()
    source_path = json_path or csv_path or ("embedded" if get_embedded_contacts else None)
        ...
        with st.container(border=True):
            ...
            if rfi_contacts:
                if contacts_csv_path == "内置通讯录":
                    source_name = "内置通讯录（rfi_contacts_embed.py）"
                else:
