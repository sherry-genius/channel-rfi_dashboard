...
def enrich_df_with_status(df):
    ...
    work["邮件内容"] = detail.apply(
        lambda d: clean_import_str(d.get("邮件摘要")) or clean_import_str(d.get("_legacy_text"))
    )
    return work

def filter_email_inbox_df(df):
    work = enrich_df_with_status(df)
    if len(work) == 0 or "同步来源" not in work.columns:
        return work.iloc[0:0].copy()
    return work[work["同步来源"] == "邮箱IMAP"].copy()
...
DEVELOPER_CHANGELOG = [
    {
        ...
        "tags": ["优化", "调单登记", "调单状态"],
        "items": [
            "「登记调单」与「调单状态」合并为「调单登记与处理」",
            "左右分栏：左侧处理邮箱调单状态，右侧手工登记调单",
            "左右栏独立滚动，互不干扰",
        ],
    },
    {
        ...
        "tags": ["优化", "调单状态"],
        "items": [
            "状态管理仅显示邮箱同步的调单，不再混入历史导入数据",
            "列表聚焦邮件标题 + 邮件正文，方便逐封处理",
        ],
    },
    {
        "version": "v1.0.21",
...
```
    ...
    with sync_col1:
        if st.button("🔄 同步邮箱", type="primary", use_container_width=True, key="email_sync_btn"):
            with st.spinner("正在拉取邮件…"):
                sync_result = sync_emails_from_imap(email_cfg)
            st.session_state["last_email_sync_result"] = sync_result
            st.rerun()
    with sync_col2:
        auto_sync = st.checkbox("打开时自动同步", value=bool(email_cfg.get("auto_sync_on_open")), key="email_auto_sync_cb")
    if not email_cfg.get("username"):
        st.caption("未配置邮箱 Secrets，右侧仍可手工登记。")

    if auto_sync and email_cfg.get("username") and email_cfg.get("password"):
        if not st.session_state.get("_email_auto_sync_done"):
            with st.spinner("自动同步…"):
                sync_result = sync_emails_from_imap(email_cfg)
...
def render_register_diaodan_panel():
    ...
    if 调单内容分类:
        st.markdown("---")
        st.markdown(f"**📌 {调单内容分类}**")
        if 调单内容分类 == "单笔交易问询":
            detail_col1, detail_col2 = st.columns(2)
            with detail_col1:
                汇款方 = st.text_input("汇款方", key="reg_remitter")
            with detail_col2:
                收款方 = st.text_input("收款方", key="reg_beneficiary")
            detail_col3, detail_col4 = st.columns(2)
            with detail_col3:
                交易类型 = st.selectbox("交易类型", TRANSACTION_TYPE_OPTIONS, key="reg_tx_type")
            with detail_col4:
                交易状态 = st.selectbox("交易状态", TRANSACTION_STATUS_OPTIONS, key="reg_tx_status")
...
def render_register_and_process_page():
    ...
    #reg-process-split + div[data-testid="stHorizontalBlock"] {
...
if page == "📊 调单看板":
    ...
    df = load_all_data()
    if len(df) == 0:
        st.info("💡 暂无数据，请先「导入历史数据」或在「调单登记与处理」中登记")
        st.stop()
...
# ============================================================
    ...
    render_register_and_process_page()
