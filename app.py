...
st.title("📋 调单管理系统")
st.warning("✅ 测试版本 v1.0.3 - 2026-07-23（数据库字段适配版）")
    ...
    for app_field, aliases in FIELD_ALIASES.items():
        for alias in aliases:
            if alias in db_col_set:
                ...
                break
    return app_to_db, db_columns

def to_db_record(app_record):
    app_to_db, _ = get_column_mapping()
    db_record = {}
    for app_field, value in app_record.items():
        db_col = app_to_db.get(app_field)
...
def save_data(收件日期, 商户ID, 商户名称, 调单类型, 金额, 币种, 业务线, 渠道, 邮件标题, 调单内容分类="", 调单内容详情=""):
    try:
        ...
        if not data:
            st.error("保存失败：数据库中未找到可写入的字段，请在 Supabase 补全表结构。")
            return False
        ...
        try:
            ...
            if not db_record:
                ...
                if len(error_samples) < 5:
                    error_samples.append(f"第 {row_idx + 2} 行：数据库无匹配字段，无法写入")
                continue
...
# ============================================================
    ...
    st.header("📤 导入历史数据")
    app_to_db, db_columns = get_column_mapping()
    with st.expander("🗄️ 数据库字段检测", expanded=not app_to_db):
        st.write("**Supabase 表 `diaodan` 当前字段：**", db_columns or "（未能自动检测，请先导入一条测试数据）")
        st.write("**已匹配的应用字段：**", app_to_db or "（无）")
        essential_fields = ["调单类型", "收件日期", "金额"]
        missing_essential = [field for field in essential_fields if field not in app_to_db]
        if missing_essential:
            st.error(f"数据库缺少关键字段映射：{missing_essential}。请在 Supabase SQL Editor 中运行以下脚本：")
            ...
            if missing:
                ...
                if test_import:
                    ...
                    if result["success_count"] > 0:
                        ...
                        st.error(f"❌ 测试写入失败：{result['error_samples'] or '未知错误'}")
                if start_import:
...
# ============================================================
    ...
    with col2:
        ...
        edited_content = st.text_area("草稿内容（可直接编辑修改）", value=default_content, height=400, key="rfi_draft")
